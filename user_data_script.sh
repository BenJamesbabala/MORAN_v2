...
VOLUME_ID=$(aws ec2 describe-volumes --region $AWS_REGION --filter Name=tag:Name,Values=Recognition-datasets-checkpoints Name=status,Values=available --query "Volumes[].VolumeId" --output text)
VOLUME_AZ=$(aws ec2 describe-volumes --region $AWS_REGION --filter Name=tag:Name,Values=Recognition-datasets-checkpoints Name=status,Values=available  --query "Volumes[].AvailabilityZone" --output text)
...
if [ $VOLUME_ID ]; then
		...
		if [ $VOLUME_AZ != $INSTANCE_AZ ]; then
				SNAPSHOT_ID=$(aws ec2 create-snapshot \
						...
						--tag-specifications 'ResourceType=snapshot,Tags=[{Key=Name,Value=Recognition-datasets-checkpoints-snapshot}]' \
